...
# logistic回归又称作logit回归、最大熵分类（maximum-entropy classification）、对数线性分类器（log-linear classifier）
# 该模型利用函数logistic function将单次试验（single trial）的可能结果输出为概率
...
# 默认情况下 lbfgs求解器鲁棒性占优
# 对于大型数据集，saga求解器通常更快
# 对于大数据集还可以用 SGDClassifier，并使用对数损失log loss可能更快，但需要更对的调优
